...
// Syntax: ./sonde_derive
//
// Input: radiosonde data (user specified, current assignment uses "DTX2016022418.txt" &
//        MHX2016022418.txt)
// 
// Output: height (m), helicity (m^2/s^2) (user specified, but must be DTX_out.txt and MHX_out.txt
//         to use gnuplot scripts)
//         height (m), temperature (C), wind chill temperature (C) (user specified, but must be
//         DTX_out_ec.txt and MHX_out_ec.txt to use gnuplot scripts)
//
...
int main() {
  ...
  char *fname_out;
  char *fname_out2;
  fname = malloc(bufsize * sizeof(char));
  fname_out = malloc(bufsize * sizeof(char));
  fname_out2 = malloc(bufsize * sizeof(char));
  if ((fname == NULL) || (fname_out == NULL) || (fname_out2 == NULL))
  {
  ...
  int stat = getline(&fname,&bufsize,stdin);
  // Remove new line character
  fname[stat-1] = '\0';
  char buff[256];		// buffer for file line read

  FILE *f_out, *f_out2;
  // Take user output files.
  printf("Please enter the name of the output file:\n");
  stat =  getline(&fname_out,&bufsize,stdin);
  // Remove new line character
  fname_out[stat-1] = '\0';
  printf("Please enter the name of the second output file:\n");
  stat = getline(&fname_out2,&bufsize,stdin);
  // Remove new line character
  fname_out2[stat-1] = '\0';
  ...
  for (i = 0; i < al; i++)
  {
    if(temp_tenth[i] != 99999 && wind_speed_tenth[i] != 99999 && wind_dir[i] != 99999)
    {
