...
#       approximation. This version plots an unstable solution, which means
#       that the errors in the calculation of the approximate solution are
#       magnified and grow over time.
#
# Syntax:
#   python oneway-unstable.py
#
#   Input: None.
#
#   Output: Interactive image file of the unstable solution.
#
# Execution Example:
#   Linux example: python oneway-unstable.py
#
# Modification History:
#   2017/11/05 - Lance Wilson:  Created from the experimental version of
#                               oneway.py, as it produces an unstable solution.
#
...
def help_message():
    print 'Syntax/Example: python oneway-unstable.py'
...
while (time < time_max):
    ...
    plt.title(r'Unstable Approximation of $\Phi$ = sin(x-ct)')
    plt.legend(loc='upper right')
    plt.pause(0.05)

    if (time == 100.0):
        plt.savefig('oneway-unstable.png', dpi=400)
