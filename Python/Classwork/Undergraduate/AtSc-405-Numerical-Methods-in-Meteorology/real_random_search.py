...
#
# Syntax:
#   python real_random_search.py winds_file
#
...
#
# Execution Example:
#   Linux example: python random_search.py winds_offset.dat
#
# Modification History:
#   2017/10/02 - Lance Wilson:  Created.
#
...
def help_message():
    print 'Syntax: python random_search.py winds_file'
    print 'Example: python random_search.py winds_offset.dat'
...
# Open random.org URL to get 1000 random floating point numbers to 20 decimal
#   places in the range [0,1].
response = urllib2.urlopen("https://www.random.org/decimal-fractions/?num=1000&dec=20&col=2&format=html&rnd=new")
# Store the HTML source code as a variable.
page_source = response.read()
# The data is also found after the "data" class header, as also formatted as
#   "%0.20f", followed by whitespace.  There are 1000 of these numbers.
values = re.search('<pre class=\"data\">((\d\.\d{20}\s){1000})</pre>', page_source)
# Store the actual data as a list, splitting on the white space characters.
numbers_split = re.split('\s', values.group(1))
# Random heading angle offsets are in degrees.
random_offsets = np.empty(1000)
# For loop is for one less than the total number of values because the last
#   value is an empty string.
for i in range(0, len(numbers_split)-1):
    # Convert numbers to range -1.0 to 1.0
    random_offsets[i] = 2.0*float(numbers_split[i]) - 1.0

standard_dev_sums = np.empty(len(random_offsets))

# Convert the heading offsets, heading, and beta angles to radians.
