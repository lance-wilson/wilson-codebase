...
#
# Syntax:
#   python classify_combined.py trained_hdf_file hdf_file_to_classify training_set band1 [bandn]
#   (Anaconda): runfile('{Path_to_file}/classify_combined.py', args='trained_hdf_file hdf_file_to_classify training_set band1 [bandn]', wdir='{Path_to_file}')
#
# Modification History:
#   2020/04/12 - Lance Wilson:  Created.
...
def help_message():
    print('Syntax: python classify_combined.py trained_hdf_file hdf_file_to_classify training_set band1 [bandn]')
    print('Syntax (Anaconda): runfile(\'{Path_to_file}/classify_combined.py\', args=\'trained_hdf_file hdf_file_to_classify training_set band1 [bandn]\', wdir=\'{Path_to_file}\')')
...
plt.savefig('data{:s}_set{:s}_{:d}bands_legend.png'.format(data_label, training_set, len(band_names)), dpi=800)

# Image with exactly one value per pixel.
img = Image.fromarray(rgbArray)
img.save('data{:s}_set{:s}_{:d}bands_no_legend.png'.format(data_label, training_set, len(band_names)))
