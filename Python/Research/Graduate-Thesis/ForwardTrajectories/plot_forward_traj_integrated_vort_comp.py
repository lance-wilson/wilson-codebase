...
#
# Name:
#   plot_forward_traj_integrated_vort_comp.py
#
# Purpose:  Plot east-west, north-south, and vertical components of vorticity
#           along CM1 forward trajectories to visualize vorticity budget data.
#
# Syntax: python3 plot_forward_traj_integrated_vort_comp.py version_number parcel_label
#
...
#
# Execution Example:
#   python3 plot_forward_traj_integrated_vort_comp.py v5 1000parcel_tornadogenesis
#
...
if len(sys.argv) > mandatory_arg_num:
    ...
    print('Syntax: python3 plot_forward_traj_integrated_vort_comp.py version_number parcel_label parcel_category number_plots')
    print('Example: python3 plot_forward_traj_integrated_vort_comp.py v5 1000parcel_tornadogenesis forward_flank 4')
...
#                                                                    #
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^#
for (plot_id, plot_index), (axis_id, (budget_coord_type, budget_var_list)) in itertools.product(enumerate(plot_indices), enumerate(sorted(budget_var_names.items()))):
    ...
    ax = fig2.add_subplot(3,1,axis_id+1, label='plot'+str(axis_id+1))
    ...
    for budget_var_name in budget_var_list:
        ...
        ax.plot(ds_obj.simulation_times[:plot_limit+1], budget_comp[:plot_limit+1], label=budget_legendlabels(budget_var_name), color=budget_linecolors(budget_var_name), linewidth=2)
    ...
    ax.plot(ds_obj.simulation_times[:plot_limit+1], prog_vort[:plot_limit+1], label='Total Prog Vort.', color='black', linestyle='dashed', linewidth=2)

    # Plot of "correct" vorticity (CM1 vorticity interpolated to trajectory
    #   positions) at each time step.
    ax.plot(ds_obj.simulation_times[:plot_limit+1], interp_vort[:plot_limit+1], label='Total Correct Vort.', color='black', linewidth=2)

    # Reference line at initial vorticity value.
    ax.plot(ds_obj.simulation_times, np.repeat(interp_vort[0], repeats=ds_obj.parcel_time_step_num), color='black', label='Init. Vort. Value')

    label_string = chr(ord('a')+axis_id)
    ax.set_title('{:s} Vorticity{:s} ({:s})'.format(dir_string, context_string, label_string))
...
for plot_id,plot_index in enumerate(plot_indices):
    ...
    image_file_name = output_dir + 'cm1_forwardtraj_vort_components_{:s}_inittime{:d}_p{:d}.png'.format(parcel_label, initialize_time, plot_index)
