...
#
# Name:
#   plot_forward_cat_traj_vectors.py
#
...
#
# Syntax: python3 plot_forward_cat_traj_vectors.py version_number parcel_label parcel_category field_variable budget_variable number_traj
#
...
#          netCDF file containing trajectories that have been categorized into
#               a source region accessed via Cat_forward_traj object from
#               categorize_forward_traj_class.py
#
# Execution Example:
#   python3 plot_forward_cat_traj_vectors.py v5 v5_meso_tornadogenesis forward_flank dbz x_stretch_term 4
#
...
if len(sys.argv) > mandatory_arg_num:
    ...
    print('Syntax: python3 plot_forward_cat_traj_vectors.py version_number parcel_label parcel_category field_variable budget_variable number_traj')
    print('Example: python3 plot_forward_cat_traj_vectors.py v5 v5_meso_tornadogenesis forward_flank dbz x_stretch_term 4')
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
nrows = int(np.ceil(np.sqrt(len(plot_file_nums))))
ncols = int(np.ceil(len(plot_file_nums)/float(nrows)))
...
for budget_var_name in budget_var_names:
    ...
    for m,cur_file_num in enumerate(plot_file_nums):
        ...
        title_base = '{:s} at Height {:d} m, Forward Trajectories Initialized at {:d} s'.format(variable_long_name, contour_height, initialize_time)

        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        # Create plots.
        #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        for n,j in enumerate(plot_indices):
            # During the first iteration, a new figure and new subplots are made.
            # During subsequent times, the numbered figure/labelled subplot is
            #   reactivated.
            fig2 = plt.figure(num=n+1, figsize=(15,13))
            ax = fig2.add_subplot(nrows, ncols, m+1, label='plot'+str(m+1))
            ...
            # Components of the horizontal vorticity and horizontal wind vectors.
            ...
            # Create the trajectory line from line segments.
            ...
            ax.annotate(chr(ord('a')+m) + ')', xy=(0.05,0.05), xycoords="axes fraction", fontsize=18)
            ax.set_title('{:d} s'.format(real_file_time))

            # Plot arrow vectors.
            Q1 = ax.quiver(x_traj_coord, y_traj_coord, xvort_comp, yvort_comp,
                            color='red', label='Horiz. Vort.', scale=0.01,
                            scale_units='xy', width=0.008)
            Q2 = ax.quiver(x_traj_coord, y_traj_coord, u_comp, v_comp,
                            color='blue', label='Horiz. Wind.', scale=10,
                            scale_units='xy', width=0.008)

            ax.quiverkey(Q1, 0.49, 0.11, 0.02, 'Horiz. Vort. (0.02 s$\mathregular{^{-1}}$)', labelpos='E', coordinates='axes')
            ax.quiverkey(Q2, 0.49, 0.04, 20, 'Horiz. Wind (20 m s$\mathregular{^{-1}}$)', labelpos='E', coordinates='axes')

            # Adjust plot so that a given distance is of equal length on both axes.
            ax.set_aspect('equal', 'datalim')

    #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    # Last pass over each trajectory for additions to the whole plot figure.
    for n,j in enumerate(plot_indices):
        fig = plt.figure(num=n+1)
        [sub_plot.set_xlabel('E-W Distance from Center (km)', fontsize = 16) for sub_plot in fig.axes[-1*ncols:]]
        [sub_plot.set_ylabel('N-S Distance from Center (km)', fontsize = 16) for sub_plot in fig.axes[::ncols]]

        # Colorbar for the background field variable.
        bar = fig.colorbar(ref, ticks=cticks, ax=fig.axes)
        bar.set_label(parameters['bar_label'], fontsize = 16)

        # Colorbar for the vorticity budget variable.
        bar2 = fig.colorbar(line, ax=fig.axes)
        bar2.set_label(budget_barlabels(budget_var_name), fontsize = 16)

        x_traj_coord = xpos[0, j]
        y_traj_coord = ypos[0, j]
        z_traj_coord = zpos[0, j]/1000.

        title_substring = '\nTrajectory {:d}, Initialized (z,y,x) (m): {:.1f}, {:.1f}, {:.1f}'.format(j, z_traj_coord*1000., y_traj_coord*1000., x_traj_coord*1000.)
        title_string = title_base + title_substring
        fig.suptitle(title_string, fontsize=18)

        fig.tight_layout(rect=[0.10, 0.03, 0.68, 0.94])

        parcel_suffix = '_p{:d}'.format(j)

        image_file_name = output_dir + 'cm1_forwardtraj_cat_vector_{:s}_{:s}_{:s}_{:s}{:s}.png'.format(parcel_label, parcel_category, budget_var_name, variable, parcel_suffix)
