...
#
# Name:
#   plot_forward_traj_vort_component.py
#
# Purpose:  Plot east-west, north-south, and vertical components of vorticity
#           tendency of representative CM1 forward trajectories.
#
# Syntax: python3 plot_forward_traj_vort_component.py version_number parcel_label
#
...
#
# Execution Example:
#   python3 plot_forward_traj_vort_component.py v5 1000parcel_tornadogenesis
#
...
if len(sys.argv) > mandatory_arg_num:
    ...
    print('Syntax: python3 plot_forward_traj_vort_component.py version_number parcel_label parcel_category number_plots')
    print('Example: python3 plot_forward_traj_vort_component.py v5 1000parcel_tornadogenesis forward_flank 4')
...
for plot_index, (budget_coord_type, budget_var_list) in itertools.product(plot_indices, budget_var_names.items()):
    ...
    for budget_var_name in budget_var_list:
        ...
        plt.plot(ds_obj.simulation_times[:plot_limit+1], budget_var[:plot_limit+1], label=budget_legendlabels(budget_var_name), color=budget_linecolors(budget_var_name))
    ...
    # Reference line at 0.
    plt.plot(ds_obj.simulation_times[:plot_limit+1], np.zeros(ds_obj.simulation_times[:plot_limit+1].shape), color='black')
    ...
    image_file_name = output_dir + 'cm1_forwardtraj_vortbudget_components_{:s}_{:s}_inittime{:d}_p{:d}.png'.format(parcel_label, budget_coord_type, initialize_time, plot_index)
