...
#
# Name:
#   plot_auto_categorized_trajectories.py
#
# Purpose:  Plot a specific group of automatically categorized CM1 backward
#           trajectories visualizing vorticity budget or height data along the
#           trajectories.
#
# Syntax: python3 plot_categorized_trajectories.py version_number parcel_label parcel_category field_variable budget_variable
#
#   Input:
#
# Execution Example:
#   python3 plot_auto_categorized_trajectories.py v5 v5_meso_tornadogenesis forward_flank dbz x_stretch_term
#
...
#                               certain category.
#   2022/09/28 - Lance Wilson:  Slight modification of plot_categorized_trajectories
#                               to adjust trajectory position arrays to work
#                               with the reduced temporal range of data in the
...
if len(sys.argv) > mandatory_arg_num:
    ...
    print('Syntax: python3 plot_auto_categorized_trajectories.py version_number parcel_label parcel_category field_variable budget_variable')
    print('Example: python3 plot_auto_categorized_trajectories.py v5 v5_meso_tornadogenesis forward_flank dbz x_stretch_term')
...
if cat_traj_obj.existing_file:
    ...
    plot_indices = cat_traj_obj.meters_to_trajnum(xpos, ypos, zpos)
else:
...
for budget_var_name in budget_var_names:
    ...
    #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    if len(plot_indices) > 0:
        # Get Data for this vorticity budget variable and create a label for
        #   the colorbar.
        budget_var = traj_ds_obj.getBudgetData(budget_var_name)[::-1][parcel_time_step_num-plot_limit:]

        # Get percentile-based limits for the colorbar, so that the range is
        #   more reasonable and not dictated by outliers.
    ...
    for cur_file_num in plot_file_nums:
        ...
        if len(plot_indices) > 0:
            ...
            for j in plot_indices:
                ...
                xpos_subset = xpos[:i+1,j]
                ypos_subset = ypos[:i+1,j]
                # Take a subset of this vorticity budget variable from the
                #   earliest time to the time currently being plotted.
                budget_var_subset = budget_var[:i+1,j]
